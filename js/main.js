...
(function() {
    getData('Cairo'); // I will invoke myself
})();
...
async function getData(cityName) {
    let response = await fetch(`https://api.weatherapi.com/v1/forecast.json?key=${apiKey}&q=${cityName}&days=3&aqi=yes&alerts=7`);
...
async function displayData() {
    ...
    <p class="text-white "> ${currentDay}</p>
    <p class="card-title text-white  ">${currentDayDate+' '+CurrentMonth}</p>
    </div>
    <div class="card-body main-color ">
       <div class="fs-1 text-white card-title mt-4 text-center">${allData.location.country}</div>
       <div class=" fs-3 text-white card-title text-center">${allData.location.name}</div>

       <div class='d-flex justify-content-around  align-items-center'>
       <div class=" fs-3 text-white text-center card-text mt-4"><h1 class="fs-1">${allData.current.temp_c} <sup>o</sup>c</h1></div>
       ...
       <div class="card-text text-warning"> ${allData.current.condition.text} </div>
       </div>
    ...
    for (var i = 1; i < 3; i++) {
        weatherInfo +=
            `     
                 <div  class="text-center  card m-md-2">
                 <div  class="card-header header-bgc  d-flex justify-content-between align-items-center">
                 <p class="text-white "> ${days[currentDayIndex+i]}</p>
                 <p class="card-title text-white ">${(currentDayDate+i)+'  '+CurrentMonth}</p>
                     ...
                     <div class="card-text text-warning"> ${allData.forecast.forecastday[i].day.condition.text} </div>
                     </div>
                </div>
                </div>
